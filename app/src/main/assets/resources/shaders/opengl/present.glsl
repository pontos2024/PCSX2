...
#define ScanBrightness -8.00f               //[-16.0 to 1.0] The overall brightness of the scanline effect. Lower for darker, higher for brighter.
#define FilterCRTAmount -1.00f              //[-4.0 to 1.0] The amount of filtering used, to replicate the TV CRT look. Lower for less, higher for more.
#define HorizontalWarp 0.00f                //[0.0 to 0.1] The distortion warping effect for the horizontal (x) axis of the screen. Use small increments.
#define VerticalWarp 0.00f                  //[0.0 to 0.1] The distortion warping effect for the verticle (y) axis of the screen. Use small increments.
#define MaskAmountDark 0.80f                //[0.0 to 1.0] The value of the dark masking line effect used. Lower for darker lower end masking, higher for brighter.
#define MaskAmountLight 1.50f               //[0.0 to 2.0] The value of the light masking line effect used. Lower for darker higher end masking, higher for brighter.
#define ResolutionScale 2.00f               //[0.1 to 4.0] The scale of the image resolution. Lowering this can give off a nice retro TV look. Raising it can clear up the image.
#define MaskResolutionScale 0.80f           //[0.1 to 2.0] The scale of the CRT mask resolution. Use this for balancing the scanline mask scale for difference resolution scaling.
...
{
	vec3 b = Fetch(pos, vec2(-1.0f, off));
	vec3 c = Fetch(pos, vec2(0.0f, off));
	vec3 d = Fetch(pos, vec2(1.0f, off));
...
{
	vec3 a = Fetch(pos, vec2(-2.0f, off));
	vec3 b = Fetch(pos, vec2(-1.0f, off));
	vec3 c = Fetch(pos, vec2(0.0f, off));
	vec3 d = Fetch(pos, vec2(1.0f, off));
	vec3 e = Fetch(pos, vec2(2.0f, off));
	float dst = Dist(pos).x;
...
vec3 Tri(vec2 pos)
{
	vec3 a = Horz3(pos, -1.0f);
	vec3 b = Horz5(pos, 0.0f);
	vec3 c = Horz3(pos, 1.0f);
...
{
	pos = pos * 2.0f - 1.0f;
	pos *= vec2(1.0f + (pos.y * pos.y) * HorizontalWarp, 1.0f + (pos.x * pos.x) * VerticalWarp);
...
vec3 Mask(vec2 pos)
{
	...
	if (fract(pos.x / 6.0f) < 0.5f)
	{
		odd = 1.0;
	}
	if (fract((pos.y + odd) / 2.0f) < 0.5f)
	{
...
vec4 LottesCRTPass()
{
	...
	vec4 color;
	vec2 inSize = u_target_resolution - (2.0f * u_target_rect.xy);
