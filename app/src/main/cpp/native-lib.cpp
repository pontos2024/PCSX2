...
Java_kr_co_iefriends_pcsx2_NativeApp_initialize(JNIEnv *env, jclass clazz,
    ...
    EmuFolders::Resources = wxDirName(_szPath);
...
Java_kr_co_iefriends_pcsx2_NativeApp_getGameTitle(JNIEnv *env, jclass clazz,
    ...
    GameList::GetGameListEntry(_szPath, &entry);
...
HostDisplay* Host::AcquireHostDisplay(HostDisplay::RenderAPI api) {
    ...
    if (!s_host_display->CreateRenderDevice(g_gs_window_info, "pcsx2", true, false)) {
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_kr_co_iefriends_pcsx2_NativeApp_saveStateToSlot(JNIEnv *env, jclass clazz, jint slot) {
    if (!VMManager::HasValidVM()) {
        return false;
    }
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_kr_co_iefriends_pcsx2_NativeApp_loadStateFromSlot(JNIEnv *env, jclass clazz, jint slot) {
    if (!VMManager::HasValidVM()) {
        return false;
    }
    ...
    if(_crc != 0) {
        ...
        if (VMManager::HasSaveStateInSlot(_serial.c_str(), _crc, slot)) {
            if(VMManager::GetState() != VMState::Paused) {
                VMManager::SetPaused(true);
            }
