...
void GSRendererHW::UpdateSettings(const Pcsx2Config2::GSOptions& old_config)
{
...
GSVector2 GSRendererHW::GetTextureScaleFactor(const bool force_upscaling)
{
	...
	{
		const float multiplier = GetUpscaleMultiplier();
...
GSVector2i GSRendererHW::GetTargetSize()
{
	...
	if (GetUpscaleMultiplier() == 1.0f || CanUpscale())
		return t_size;
...
void GSRendererHW::EmulateBlending(bool& DATE_PRIMID, bool& DATE_BARRIER, bool& blending_alpha_pass)
{
	...
	else if (sw_blending)
	{
		...
		HWBlend blend = GSDevice::GetBlend(blend_index, replace_dual_src);
		...
		{
            if(GSConfig.Renderer == GSRendererType::VK)
            {
                // No sw blending
                m_conf.ps.blend_a = m_conf.ps.blend_b = m_conf.ps.blend_d = 0;
                ...
                if (m_conf.ps.dfmt == 1 && m_conf.ps.blend_c == 1)
                {
                    ...
                    blend = GSDevice::GetBlend(blend_index + 3, replace_dual_src); // +3 <=> +1 on C
