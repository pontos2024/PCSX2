...
void VMManager::ApplyGameFixes()
{
	...
	{
		auto eeRM = (SSE_RoundMode)enum_cast(game->eeRoundMode);
		if (EnumIsValid(eeRM))
		{
	...
	{
		auto vuRM = (SSE_RoundMode)enum_cast(game->vuRoundMode);
		if (EnumIsValid(vuRM))
		{
	...
	// TODO - config - this could be simplified with maps instead of bitfields and enums
    for (const std::string& fix : game->gameFixes)
	{
