...
void SysMtgsThread::PostVsyncStart(bool registers_written)
{
	...
	uint packsize = sizeof(RingCmdPacket_Vsync) >> 4;
	...
	if ((m_QueuedFrameCount.fetch_add(1) < EmuConfig2.GS.VsyncQueueSize) /*|| (!EmuConfig2.GS.VsyncEnable && !EmuConfig2.GS.FrameLimitEnable)*/)
		return;
...
bool SysMtgsThread::OpenGS()
{
    memcpy(RingBuffer.Regs, PS2MEM_GS, sizeof(PS2MEM_GS));

	if (!GSopen(EmuConfig2.GS, EmuConfig2.GS.Renderer, RingBuffer.Regs)) {
        return false;
    }
        ...
        while (m_ReadPos.load(std::memory_order_relaxed) != m_WritePos.load(std::memory_order_acquire))
        {
			...
			switch (tag.command)
			{
				...
				case GS_RINGTYPE_GSPACKET:
				{
					...
					u32 size = tag.data[1];
					if (offset != ~0u)
						GSgifTransfer((u8*)&path.buffer[offset], size >> 4);
				...
				case GS_RINGTYPE_MTVU_GSPACKET:
				{
					...
					if (gsPack.size)
						GSgifTransfer((u8*)&path.buffer[gsPack.offset], gsPack.size >> 4);
				...
				default:
				{
					switch (tag.command)
					{
						...
						case GS_RINGTYPE_ASYNC_CALL:
							{
								auto* const func = (AsyncCallType*)tag.pointer;
								(*func)();
								delete func;
							}
							break;
						...
						{
							auto* data = (MTGS_FreezeData*)tag.pointer;
							int mode = tag.data[0];
...
#endif
                            GSInitAndReadFIFO((u8*)tag.pointer, 1);
							break;
...
//							pxFail("Bad packet encountered in the MTGS Ringbuffer.");
			...
			if (EmuConfig2.GS.SynchronousMTGS)
			{
...
void SysMtgsThread::SendDataPacket()
{
	...
	auto& tag = (PacketTagType&)RingBuffer[m_packet_startpos];
...
void SysMtgsThread::GenericStall(uint size)
{
	...
	if (freeroom <= size)
	{
		...
		uint somedone = (RingBufferSize - freeroom) >> 2;
		if (somedone < size + 1)
			somedone = size + 1;
...
void SysMtgsThread::PrepDataPacket(MTGS_RingCommand cmd, u32 size)
{
	...
	auto& tag = (PacketTagType&)RingBuffer[local_WritePos];
	tag.command = cmd;
...
void SysMtgsThread::SendSimplePacket(MTGS_RingCommand type, int data0, int data1, int data2)
{
	...
	GenericStall(1);
	auto& tag = (PacketTagType&)RingBuffer[m_WritePos.load(std::memory_order_relaxed)];
...
void SysMtgsThread::ApplySettings()
{
	...
	RunOnGSThread([opts = EmuConfig2.GS]() {
		GSUpdateConfig(opts);
	});
    ...
    if (EmuConfig2.GS.HWDisableReadbacks)
...
void SysMtgsThread::SwitchRenderer(GSRendererType renderer)
{
	...
	RunOnGSThread([renderer]() {
		Host::AddOSDMessage(StringUtil::StdStringFromFormat("Switching to %s renderer...", Pcsx2Config::GSOptions::GetRendererName(renderer)), 2.0f);
...
void SysMtgsThread::SetSoftwareRendering(bool software)
{
	...
	if (!software)
		new_renderer = EmuConfig2.GS.UseHardwareRenderer() ? EmuConfig2.GS.Renderer : GSRendererType::Auto;
	else
