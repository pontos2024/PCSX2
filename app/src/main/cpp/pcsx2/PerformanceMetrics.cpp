...
void PerformanceMetrics::Update(bool gs_register_write, bool fb_blit)
{
	...
	if (s_gs_privileged_register_writes_since_last_update > 0 && !EmuConfig2.Gamefixes.BlitInternalFPSHack)
	{
	...
	{
		const u64 time2 = thread.handle.GetCPUTime();
		const u64 delta = time2 - thread.last_cpu_time;
		thread.last_cpu_time = time2;
