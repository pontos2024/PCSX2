...
bool ImGuiManager::Initialize()
{
	...
	s_global_scale = std::max(1.0f, display->GetWindowScale() * static_cast<float>(EmuConfig2.GS.OsdScale / 100.0));
...
void ImGuiManager::UpdateScale()
{
	...
	const float scale = std::max(window_scale * static_cast<float>(EmuConfig2.GS.OsdScale / 100.0), 1.0f);
...
//			}
		...
		if (GSConfig.OsdShowSpeed)
		{
			...
			if (EmuConfig2.GS.LimitScalar == 0.0)
				text += " (Max)";
			else
				fmt::format_to(std::back_inserter(text), " ({:.0f}%)", EmuConfig2.GS.LimitScalar * 100.0);
		...
		if (GSConfig.OsdShowCPU)
		{
			...
			text.clear();
			if (EmuConfig2.Speedhacks.EECycleRate != 0 || EmuConfig2.Speedhacks.EECycleSkip != 0)
				fmt::format_to(std::back_inserter(text), "EE[{}/{}]: ", EmuConfig2.Speedhacks.EECycleRate, EmuConfig2.Speedhacks.EECycleSkip);
			else
				text = "EE: ";
...
//		}
		...
		{
			const bool is_normal_speed = (EmuConfig2.GS.LimitScalar == EmuConfig2.Framerate.NominalScalar);
			if (!is_normal_speed)
			{
				const bool is_slowmo = (EmuConfig2.GS.LimitScalar < EmuConfig2.Framerate.NominalScalar);
